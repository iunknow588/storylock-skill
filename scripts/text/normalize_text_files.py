#!/usr/bin/env python3
"""
Normalize repository text files to UTF-8 (no BOM) and LF.
...
- Common generated and dependency directories are skipped.
...
def process_file(path: Path, fix: bool) -> Optional[str]:
    ...
    if changed:
        ...
        if enc.lower() not in {"utf-8", "ascii"}:
            reasons.append(f"encoding={enc}")
        if data.startswith(b"\xef\xbb\xbf"):
            reasons.append("bom")
        if b"\r\n" in data or b"\r" in data:
            reasons.append("line_endings")
        detail = ", ".join(reasons) if reasons else f"encoding={enc}"
        return f"CHANGED ({detail})"
    else:
        return "OK"
