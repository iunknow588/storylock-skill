...
a three-layer coordinated Skill solution;
an Agent security solution in which a local Agent manages private keys and file keys, associative-memory cues participate in authorization decisions, and authorization is executed by permission level;
a verifiable project with code, documentation, example scripts, and self-test scripts.
