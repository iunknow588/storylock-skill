...
| AND-09 | 脱敏返回 | 真机/接口 | `/api/storylock-gateway` | password、privateKey、signingKeyBytes 不明文返回 |
...
| LIN-02 | SecretStore 适配 | 自动/环境 | `node src/skills/local-story-access/scripts/check-secret-store.mjs` | Linux 真环境中 `secret-tool` 与 Secret Service 可用 |
| LIN-03 | 原型服务启动 | 自动/本地 | `node src/host/linux-host/server.mjs` | 服务监听 `127.0.0.1` 并返回 `/health` |
| LIN-04 | 题库导入 | 自动/本地 | `POST /question-bank/import` | 支持 UTF-8/BOM JSON，导入后 active 题数不少于 9 |
...
| LIN-12 | 生产限制 | 人工/文档 | `docs/ref/11-Linux平台密钥存储与检查说明.md` | 不把原型 host 宣称为正式 Linux 发布闭环 |
