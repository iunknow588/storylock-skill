...
if (master.status === 'active' && master.questions.filter((question) => question.status === 'active').length < 9) {
  fail('Active question set imports require at least 9 active questions for high-strength grid verification.');
}
...
if (dryRun) {
  console.log(JSON.stringify({
    ...
    activeQuestionCount: master.questions.filter((question) => question.status === 'active').length,
  }, null, 2));
  process.exit(0);
}
