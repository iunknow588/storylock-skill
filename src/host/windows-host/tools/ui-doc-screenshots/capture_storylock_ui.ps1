...
$windowsHostRoot = Resolve-Path (Join-Path $PSScriptRoot "..\..")
$skillRoot = Resolve-Path (Join-Path $PSScriptRoot "..\..\..\..\..")

if ([string]::IsNullOrWhiteSpace($ExePath)) {
    $ExePath = Join-Path $windowsHostRoot "target\codex-build\debug\yian-windows-host.exe"
}

if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    $OutputDir = Join-Path $skillRoot "docs\management\流程图\ui-screenshots"
}
