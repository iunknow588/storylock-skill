...
$modulePath = Join-Path $moduleDir "callbacks.rs"
...
for ($i = $startLine - 1; $i -lt $endLine; $i++) {
    $line = $lines[$i]
    if ($line -match '^pub\(super\) fn wire_storylock_core_callbacks') {
        $line = $line
    }
    $extracted.Add($line)
}
...
$newRoot.Add("mod callbacks;")
$newRoot.Add("use callbacks::*;")
$newRoot.Add("")
...
Write-Output "storylock callbacks extracted"
