...
function responseStub({ ok = true, status = 200, body = '' } = {}) {
  return {
    ok,
    status,
    async text() {
...
const successfulTransport = createHttpRemoteTransport({
  endpoint: 'https://gateway.example.test/storylock',
  fetchImpl: async (url, init) => {
    assert.equal(url, 'https://gateway.example.test/storylock');
    assert.equal(init.method, 'POST');
    assert.equal(init.headers['content-type'], 'application/json; charset=utf-8');
    assert.deepEqual(JSON.parse(init.body), { requestId: 'req-http-ok' });
    return responseStub({ body: '{"ok":true}' });
  },
});
assert.deepEqual(await successfulTransport({ requestId: 'req-http-ok' }), { ok: true });

const emptyTransport = createHttpRemoteTransport({
  endpoint: 'https://gateway.example.test/storylock',
  fetchImpl: async () => responseStub(),
});
assert.equal(await emptyTransport({ requestId: 'req-http-empty' }), null);

await assert.rejects(
  () => createHttpRemoteTransport({
    endpoint: 'https://gateway.example.test/storylock',
    fetchImpl: async () => responseStub({ body: 'not json' }),
  })({ requestId: 'req-http-non-json' }),
  /remote gateway returned non-JSON response/,
);
...
await assert.rejects(
  () => createHttpRemoteTransport({
    ...
    fetchImpl: async () => responseStub({
      ok: false,
      status: 403,
      body: '{"error":{"message":"forbidden by policy"}}',
    }),
  })({ requestId: 'req-http-error-json' }),
  /forbidden by policy/,
);
...
await assert.rejects(
  () => createHttpRemoteTransport({
    endpoint: 'https://gateway.example.test/storylock',
    fetchImpl: async () => responseStub({ ok: false, status: 503 }),
  })({ requestId: 'req-http-error-empty' }),
  /remote gateway request failed with status 503/,
);
...
await assert.rejects(
  ...
  })({ requestId: 'req-http-error-html' }),
  /remote gateway request failed with status 502/,
);
...
await assert.rejects(
  ...
  /remote gateway request timed out after 1ms/,
);
